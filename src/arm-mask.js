...
export const LINKS=[[0,1],[1,2],[2,3],[3,4],[0,5],[5,6],[6,7],[7,8],[5,9],[9,10],[10,11],[11,12],[9,13],[13,14],[14,15],[15,16],[13,17],[0,17],[17,18],[18,19],[19,20]];
...
function segmentDistance(point,a,b){
  const dx=b.x-a.x,dy=b.y-a.y,t=clamp(((point.x-a.x)*dx+(point.y-a.y)*dy)/Math.max(dx*dx+dy*dy,1e-9),0,1);
  return Math.hypot(point.x-a.x-dx*t,point.y-a.y-dy*t);
}

function armGeometry(landmarks){
  const scale=handScale(landmarks);if(!scale)return null;
  const wrist=landmarks[0],palm=[5,9,13,17].reduce((sum,index)=>({x:sum.x+landmarks[index].x/4,y:sum.y+landmarks[index].y/4}),{x:0,y:0});
  let dx=wrist.x-palm.x,dy=wrist.y-palm.y,length=Math.hypot(dx,dy);
  if(length<.01){dx=0;dy=1;length=1;}
  dx/=length;dy/=length;
  const candidates=[];
  if(dx>0)candidates.push((1-wrist.x)/dx);else if(dx<0)candidates.push(-wrist.x/dx);
  if(dy>0)candidates.push((1-wrist.y)/dy);else if(dy<0)candidates.push(-wrist.y/dy);
  const valid=candidates.filter(value=>Number.isFinite(value)&&value>0),travel=valid.length?Math.min(...valid):1;
  return {landmarks,palm,wrist,end:{x:wrist.x+dx*travel,y:wrist.y+dy*travel},scale};
}
...
export class ArmAnchor{
  constructor({maxCredit=26,bottomSpan=.38,sideStart=.55}={}){
    this.maxCredit=maxCredit;this.bottomSpan=bottomSpan;this.sideStart=sideStart;this.credit=null;
  }
  update(person,width,height){
    if(!this.credit||this.credit.length!==person.length)this.credit=new Uint8Array(person.length);
    const credit=this.credit,anchored=new Uint8Array(person.length),connected=new Uint8Array(person.length),stack=[];
    const seed=(index,map)=>{if(person[index]&&!map[index]){map[index]=1;stack.push(index);}};
    const fill=map=>{
      while(stack.length){
        const index=stack.pop(),x=index%width,y=(index-x)/width;
        if(x>0)seed(index-1,map);if(x<width-1)seed(index+1,map);
        if(y>0)seed(index-width,map);if(y<height-1)seed(index+width,map);
      }
    };
    for(let x=0;x<width;x++)if(x<=width*this.bottomSpan||x>=width*(1-this.bottomSpan))seed((height-1)*width+x,anchored);
    for(let y=Math.floor(height*this.sideStart);y<height;y++){seed(y*width,anchored);seed(y*width+width-1,anchored);}
    fill(anchored);
    for(let i=0;i<person.length;i++){
      if(anchored[i])connected[i]=1;
      else if(credit[i]&&person[i]){connected[i]=1;stack.push(i);}
    }
    fill(connected);
    for(let i=0;i<credit.length;i++)credit[i]=anchored[i]?this.maxCredit:(credit[i]?credit[i]-1:0);
...
export function armCutout(labels,width,height,hands,support,anchor){
  const alpha=new Uint8ClampedArray(width*height),person=new Uint8Array(alpha.length);
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    const label=labels[y*width+x];if(label!==2&&label!==4)continue;
    for(let oy=-1;oy<=1;oy++)for(let ox=-1;ox<=1;ox++){
      const px=x+ox,py=y+oy;if(px<0||px>=width||py<0||py>=height)continue;
  ...
  const anchoredHands=geometry.map(arm=>{
    ...
    for(const index of [0,5,9,13,17]){
      ...
      const px=Math.min(width-1,Math.max(0,Math.round(point.x*width-.5))),py=Math.min(height-1,Math.max(0,Math.round(point.y*height-.5)));
      for(let oy=-1;oy<=1;oy++)for(let ox=-1;ox<=1;ox++){
        const qx=px+ox,qy=py+oy;if(qx<0||qx>=width||qy<0||qy>=height)continue;
  ...
  for(let armIndex=0;armIndex<geometry.length;armIndex++){
    ...
    const x0=Math.max(0,Math.floor((minX-arm.scale)*width)),x1=Math.min(width-1,Math.ceil((maxX+arm.scale)*width));
    const y0=Math.max(0,Math.floor((minY-arm.scale)*height)),y1=Math.min(height-1,Math.ceil((maxY+arm.scale)*height));
    const dx=arm.end.x-arm.wrist.x,dy=arm.end.y-arm.wrist.y,lengthSq=Math.max(dx*dx+dy*dy,1e-9);
    for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){
      const index=y*width+x;if(alpha[index]===255)continue;
      const p={x:(x+.5)/width,y:(y+.5)/height};
      const backed=person[index]&&(!region||region.connected[index]);
      if(backed){
        const t=((p.x-arm.wrist.x)*dx+(p.y-arm.wrist.y)*dy)/lengthSq;
        if(t>=-.12&&t<=1.04&&segmentDistance(p,arm.wrist,arm.end)<=arm.scale*(.42+.55*clamp(t,0,1))){alpha[index]=255;continue;}
      }
      let inHand=distance(p,arm.palm)<=arm.scale*(backed?.72:.6);
      if(!inHand){const reach=arm.scale*(backed?.13:.11);for(const [a,b] of LINKS)if(segmentDistance(p,arm.landmarks[a],arm.landmarks[b])<=reach){inHand=true;break;}}
      if(inHand)alpha[index]=backed?255:Math.max(alpha[index],FALLBACK_ALPHA);
    }
  }
  ...
  if(support&&support.length===alpha.length){
    for(let y=0;y<height;y++)for(let x=0;x<width;x++){
      ...
      if(alpha[index]===255||!person[index]||(region&&!region.connected[index]))continue;
      let near=false;
      for(let oy=-1;oy<=1&&!near;oy++)for(let ox=-1;ox<=1;ox++){
        const px=x+ox,py=y+oy;if(px<0||px>=width||py<0||py>=height)continue;
        if(support[py*width+px]>=80){near=true;break;}
      }
...
export function arcCoverage(alpha,width,height,target,radius){
  const r=radius*height,cx=target.x*width,cy=target.y*height;
...
export class MaskSmoother{
  constructor({attack=.65,decay=.3}={}){this.attack=attack;this.decay=decay;this.state=null;}
  apply(alpha){
    if(!this.state||this.state.length!==alpha.length){this.state=new Uint8ClampedArray(alpha);return this.state;}
    const state=this.state;
    for(let i=0;i<alpha.length;i++){
      const target=alpha[i],current=state[i];
      const next=current+(target-current)*(target>current?this.attack:this.decay);
