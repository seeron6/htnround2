...
export function classifyMode(
  ...
  const verdict = (mode, why) => {
    if (detail) detail.why = why;
    return mode;
  };
  ...
  const faceOn = facing >= 0.55,
    edgeOn = !!handAxis && facing < 0.55;
  const axisLen = handAxis ? Math.hypot(handAxis.du, handAxis.dv) : 0;
  const axisAimed = handAxis && axisLen < 0.32; // knuckles at the camera
  const axisUp =
    handAxis &&
    faceOn &&
    handAxis.dv > 0.4 &&
    handAxis.dv > Math.abs(handAxis.du) * 1.3;
  const axisDown =
    handAxis &&
    faceOn &&
    handAxis.dv < -0.4 &&
  ...
  const axisSide =
    handAxis &&
    faceOn &&
    Math.abs(handAxis.du) > 0.55 &&
    Math.abs(handAxis.du) > Math.abs(handAxis.dv) * 1.3;
  const d = direction,
  ...
  const hookEvidence = !sweep || su >= 0.06 || axisSide;
  ...
  const closeRange = Number.isFinite(range) && range < 0.25;
  const orientationUp = handAxis ? axisUp : !!n && n[1] > 0.75;
  const upOk = rose || (!closeRange && orientationUp);
  if (axisUp && upOk && !(lat > 0.6 && sweepHook))
    return verdict('uppercut', 'fist-points-up');
  if (axisDown && Math.abs(d[0]) < 0.6 && (sweep?.dv ?? 0) < -0.08)
    return verdict('overhand', 'fist-points-down');
  if (
    !knAtCamera &&
    !edgeOn &&
    sweepUp &&
    ...
    return verdict('uppercut', 'image-rise');
  ...
  if (lat > 0.6 && d[1] < 0.6 && hookEvidence) return verdict('hook', 'lateral-travel');
  ...
  if (axisSide && lat > 0.3 && !sweepUp) return verdict('hook', 'fist-points-side');
  if (n) {
    ...
    if (
      ...
      return verdict('uppercut', 'knuckles+rise');
    ...
    if (upOk && !edgeOn && n[1] > 0.75 && d[1] > -0.15 && lat < 0.55 && !sweepHook)
      return verdict('uppercut', 'knuckles-vertical');
    if (n[1] < -0.5 && d[1] < -0.2) return verdict('overhand', 'knuckles-down');
    if (Math.abs(n[0]) > 0.5 && lat > 0.3 && hookEvidence)
      return verdict('hook', 'knuckles-side');
  }
  if (!knAtCamera && !edgeOn && upOk && d[1] > 0.45 && d[1] > lat * 0.9 && !sweepHook)
    return verdict('uppercut', 'rising-travel');
  ...
  if (
    ...
    return verdict('uppercut', 'from-below');
  if (lat > 0.45 && hookEvidence) return verdict('hook', 'lateral');
  if (lat > 0.3 && (curvature > 0.2 || sweepHook) && hookEvidence)
    return verdict('hook', 'arc');
  if (sweepHook && lat > 0.22) return verdict('hook', 'image-sweep');
...
export function resolveHand({
  ...
  if (Number.isFinite(restU) && Math.abs(restU - 0.5) > 0.06)
    cast(restU < 0.5, 1.4 * Math.min(1, Math.abs(restU - 0.5) / 0.18));
  if (Number.isFinite(otherRestU) && Math.abs(otherRestU - 0.5) > 0.06)
    cast(otherRestU >= 0.5, 1.2 * Math.min(1, Math.abs(otherRestU - 0.5) / 0.18));
  ...
  for (const { label, score } of labels) {
    ...
    if (label === 'Right') labelScore += score ?? 0.5;
    else if (label === 'Left') labelScore -= score ?? 0.5;
    labelCount++;
  }
...
export const DEFAULTS = {
  ...
  strikeRange: 0.45, // m   — a punch ARRIVES: its apex must come within striking range of the
...
export const EDGE_MARGIN = 0.14,
  EDGE_PEAK = 0.35,
  EDGE_REACH = 0.035; // EDGE_PEAK rides the same
...
const REST_SPEED = 0.45; // image speed (u/s) below which a fist counts as resting
...
const REST_ALPHA = 0.12; // EMA rate of the learned rest position
const RIGID_SUPPORT = 0.4; // fraction of a candidate's claimed travel that must be witnessed by
...
function coexisted(a, b) {
  ...
  return (
    Math.min(a.lastObsT, b.lastObsT) - Math.max(a.firstObsT, b.firstObsT) >= COEXIST_MS
  );
}
...
class Slot {
  constructor(u, v, time) {
    ...
    this.pdu = 0;
    this.pdv = 0;
    this.pt = time; // blob-informed predicted position
    this.firstObsT = null;
    this.lastObsT = null; // landmark span, for coexistence
    this.restU = null;
    this.restW = 0;
    this.slowRun = 0; // guard lineage (see push)
    this.bornAtEdge =
  ...
  predictAt(t) {
    const dt = clamp((t - this.pt) / 1000, 0, 0.5);
...
export class PunchExtractor {
    ...
    if (dt > 1e-3 && dt < 0.5) {
      ...
      if (Math.hypot(slot.du, slot.dv) < REST_SPEED) {
        if (++slot.slowRun >= REST_RUN) {
          slot.restU =
            slot.restU === null ? u : slot.restU + (u - slot.restU) * REST_ALPHA;
  ...
  #extract(slot, now) {
    ...
    const rigidWindup = slot.samples.filter(
      (s) =>
        s.quality === 'rigid' && s.t >= windupFloor && s.t <= at.t + RIGID_CONFIRM_MS,
    );
    ...
    if (rigidTravel < travel * RIGID_SUPPORT && rigidTravel < o.minTravel)
      return reject(
        `approach unverified (${(rigidTravel * 100).toFixed(0)} of ${(travel * 100).toFixed(0)}cm rigid)`,
      );
    ...
    const arrive =
      kind === 'censored' ? at.r - trailingClosing * (IMMINENT_MS / 1000) : at.r;
    if (arrive > o.strikeRange)
      return reject(
        `stopped ${(at.r * 100).toFixed(0)}cm from the head (> ${(o.strikeRange * 100).toFixed(0)}cm)`,
      );
    if (kind === 'apex') {
  ...
  #suppressed(candidate, now) {
    for (const e of this.recent) {
      const apart = Math.abs(candidate.at.t - e.t),
        age = now - e.emittedAt;
      if (
        (apart <= SAME_REGION_MS || age <= SAME_REGION_MS) &&
        Math.hypot(candidate.at.s.u - e.u, candidate.at.s.v - e.v) <= MERGE_GATE
      )
        return true;
      if (
        (apart <= SAME_FIST_MS || age <= SAME_FIST_MS) &&
        e.slot &&
        !(e.complete && candidate.kind === 'apex') &&
        !coexisted(candidate.slot, e.slot)
      )
        return true;
    }
    return false;
  ...
  #solve(candidate, { gain, headRadii, now }) {
    ...
    const startP =
      stateAt(trail, tStart, FIT_HALF_MS)?.position ?? candidate.fitted[0].s.p;
    const chord = (() => {
    ...
    let axisU = 0,
      ...
      axisW = 0;
    for (const s of slot.samples) {
      if (s.t < at.t - TERMINAL_MS || s.t > at.t + TERMINAL_AFTER_MS || !s.axis)
        continue;
      axisU += s.axis.du * s.w;
      axisV += s.axis.dv * s.w;
      axisF += (s.axis.facing ?? 1) * s.w;
      axisW += s.w;
    }
    const handAxis = axisW
      ? { du: axisU / axisW, dv: axisV / axisW, facing: axisF / axisW }
      : null;
    ...
    let su = at.s.u - first.s.u,
      sv = at.s.v - first.s.v;
    const mu = (first.s.u + at.s.u) / 2 - 0.5,
      mv = (first.s.v + at.s.v) / 2 - 0.5,
      away = Math.hypot(mu, mv);
    if (away >= 0.08) {
    ...
    if (restU === null)
      for (const other of this.slots.values()) {
        if (other === slot || other.restW < REST_MIN_W || other.slowRun < REST_RUN)
          continue;
    ...
    for (const candidate of candidates) {
      ...
      if (emitted) {
        ...
        if (
          ...
          (!(emitted.kind === 'apex' && candidate.kind === 'apex') &&
            !coexisted(candidate.slot, emitted.slot))
        ) {
          this.#consume(candidate, candidate.at.t);
          this.stats.merged++;
        }
