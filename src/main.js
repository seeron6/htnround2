...
function installHair(spec) {
  ...
  if (headHairCap) {
    headHairCap.visible = false;
    headPivot.add(headHairCap);
  }
...
function restoreAccessoryVisibility(accessories) {
  ...
  if (headHair) {
    ...
    if (headHairCap) headHairCap.visible = false;
...
function setView(view) {
  ...
  if (headHairCap && headHair) headHairCap.visible = false;
  if (headHair) headHair.visible = view !== 'clay' && $('hair-visible').checked;
...
$('distance').oninput = () => {
  ...
  tracking.setTargetDistance(Number($('distance').value));
...
async function loadNativePreview(id) {
  ...
  try {
    ...
    gltf.scene.traverse((object) => {
      if (
        object.userData.accessory === 'eyeglasses' ||
        object.userData.accessory === 'hair'
      )
        serializedAccessories.push(object);
      if (object.isMesh) meshes.push(object);
    });
    // A published GLB may contain an earlier serialized accessory pass plus
    // the authoritative metadata on the head. Remove the imported accessory
    // nodes before rebuilding the rigid eyewear/hair groups so the glasses
    // cannot be duplicated or become part of the deformable head path.
    for (const object of serializedAccessories)
