...
class Axis {
  ...
  step(dt) {
    ...
    const f = (r, c) => P[r * 3 + c];
    const m = [
      f(0, 0) + dt * f(1, 0) + h * f(2, 0),
      f(0, 1) + dt * f(1, 1) + h * f(2, 1),
      f(0, 2) + dt * f(1, 2) + h * f(2, 2),
      f(1, 0) + dt * f(2, 0),
      f(1, 1) + dt * f(2, 1),
      f(1, 2) + dt * f(2, 2),
      f(2, 0),
  ...
  at(dt) {
    const [p, v, a] = this.x;
    return [p + v * dt + (a * dt * dt) / 2, v + a * dt, a];
  }
}
...
export class PoseFilter {
  ...
  constructor({
    ...
    this.orientationTau = orientationTau;
  ...
  update(position, variance, timestamp, quaternion) {
    if (
      ...
      return false;
    const raw =
      Array.isArray(variance) && variance.every(Number.isFinite)
        ? variance
        : [1e-4, 1e-4, 1e-3];
    ...
    if (
      !this.axes ||
      this.time === null ||
      timestamp - this.time > this.maxGapMs ||
      timestamp < this.time
    ) {
      this.axes = [0, 1, 2].map((k) => {
        const axis = new Axis(this.jerk);
        axis.reset(position[k], Math.max(v[k], 1e-8));
        return axis;
      });
      this.time = timestamp;
      this.quaternion = quaternion ? [...quaternion] : null;
      this.started = true;
      return true;
    }
    ...
    for (let k = 0; k < 3; k++) {
      const axis = this.axes[k],
        predicted = axis.at(dt)[0],
        s2 = axis.P[0] + Math.max(v[k], 1e-8);
    ...
    this.boost = clamp(
      Math.max(this.boost * 0.55, Math.sqrt(Math.max(nis - this.trigger, 0))),
      1,
      this.agility,
    );
    for (let k = 0; k < 3; k++) {
      ...
      this.axes[k].step(dt);
      this.axes[k].correct(position[k], Math.max(v[k], 1e-8));
    }
    this.time = timestamp;
    if (quaternion)
      this.quaternion = this.quaternion
        ? slerp(this.quaternion, quaternion, 1 - Math.exp(-dt / this.orientationTau))
        : [...quaternion];
    return true;
  }
  ...
  at(timestamp) {
    ...
    const dt = clamp(
      ((Number.isFinite(timestamp) ? timestamp : this.time) - this.time) / 1000,
      -0.05,
      0.2,
    );
    const s = this.axes.map((axis) => axis.at(dt));
    return {
      position: s.map((v) => v[0]),
      velocity: s.map((v) => v[1]),
      acceleration: s.map((v) => v[2]),
      quaternion: this.quaternion,
    };
  }
  get speed() {
    const s = this.at(this.time);
    return s ? Math.hypot(...s.velocity) : 0;
  }
}
