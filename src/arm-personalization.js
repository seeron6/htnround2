...
export function sampleArmFrame({ data, width, height }, landmarks, pose) {
  const samples = {};
  const usedHands = new Set();
  const aspect = width / height;
  if (!pose || !landmarks?.length) return samples;
  const pixel = (x, y) => {
    const i =
  ...
  for (const [side, elbowId, wristId] of [
    ...
    const elbow = pose[elbowId],
      wrist = pose[wristId];
    if (!valid(elbow) || !valid(wrist)) continue;
    const hand = landmarks.reduce(
      (best, h) =>
        !usedHands.has(h) &&
        valid(h?.[0]) &&
        (!best || dist(h[0], wrist, aspect) < dist(best[0], wrist, aspect))
          ? h
          : best,
      null,
    );
    if (
      !hand ||
      dist(hand[0], wrist, aspect) > 0.1 ||
      ![0, 5, 9, 17].every((i) => valid(hand[i]))
    )
      continue;
    const span = dist(hand[5], hand[17], aspect);
    const length = dist(wrist, elbow, aspect);
    if (span < 0.045 || length < span * 1.4 || length > span * 7) continue;
    usedHands.add(hand);
    const palm = {
    ...
    if (Math.max(...skin) < 8 || Math.min(...skin) > 250) continue;
    const stripWidth = 64,
      stripHeight = 192;
...
export class ArmScanAccumulator {
  ...
  finish() {
    ...
    if (missing.length)
      throw new Error(
        `Show your ${missing.join(' and ')} forearm from elbow to fingertips, away from your body, then try again.`,
      );
