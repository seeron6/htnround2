...
export class QuickArms {
  constructor({ tracking, camera, scene, setMode, getMode, onChange = () => {} }) {
    ...
    this.dialog.innerHTML = /* HTML */ ` <div class="quick-arm-content">
        ...
        <p class="quick-arm-intro">
          Scan your skin, sleeves and visible accessories. The 8-second countdown starts
          when both arms are visible. Move back to show your face, shoulders, elbows and
          open hands, then slowly turn your forearms.
        </p>
        <div class="quick-arm-layout">
          <div class="quick-arm-visuals">
            <div id="quick-arm-camera" hidden>
              <video
                id="quick-arm-video"
                playsinline
                muted
              ...
              ></video>
              <div class="quick-arm-feedback" aria-label="Arm visibility">
                <p id="quick-arm-left"></p>
                <p id="quick-arm-right"></p>
              </div>
            </div>
  ...
  get state() {
    return {
      ...
      phase: this.running ? (this.waiting ? 'framing' : 'capturing') : 'idle',
  ...
  async open() {
    ...
    this.$('camera').hidden = !this.tracking.active;
    this.feedback = { left: 'waiting', right: 'waiting' };
    this.updateFeedback();
    this.$('video')
      .play()
      .catch(() => {});
    ...
    if (ready)
      this.status(
        'Ready. Press Scan, then use the camera preview to bring both elbows and open hands into view. The countdown waits for you.',
      );
  ...
  updateFeedback() {
    for (const side of sides) {
      const reason = this.feedback[side];
      const label = side === 'left' ? 'Left' : 'Right';
      const count = this.accumulator.frames[side];
      this.$(side).textContent =
        `${label}: ${ARM_SCAN_HINTS[reason]}${this.running && !this.waiting ? ` (${count} views)` : ''}`;
      this.$(side).dataset.state = reason === 'ready' ? 'ready' : 'waiting';
    }
  }
  syncFields() {
  ...
  async start() {
    ...
    this.updateFeedback();
    this.dialog.querySelector('.quick-arm-content').scrollTop = 0;
    this.status(
      'Position both arms in the camera preview. The 8-second countdown has not started.',
    );
    this.timer = setInterval(() => {
      if (!this.tracking.active)
        return this.cancel('Camera disconnected. Reconnect it, then reopen the scan.');
      if (this.waiting) {
        if (performance.now() - this.requestedAt > 30000)
          this.cancel(
            'No scan captured. Follow the guidance beside each arm, then try again. Your previous arms are kept.',
          );
        return;
      }
  ...
  setScanning(active) {
    this.dialog.dataset.scanning = String(active);
    if (active) this.$('progress').value = 0;
  ...
  accept(data) {
    if (!this.running || data.timestamp < (this.started ?? this.requestedAt)) return;
    const pose = data.pose;
    this.feedback = {};
    const samples = sampleArmFrame(
      data.armSample,
      data.landmarks,
      pose && Math.abs(data.timestamp - pose.timestamp) <= 100
        ? pose.landmarks?.[0]
        : null,
      this.feedback,
    );
    if (this.waiting) {
      ...
      this.status(
        'Both arms detected. Scanning for 8 seconds — slowly turn your forearms.',
      );
    }
  ...
  finish() {
    ...
    try {
      ...
      const profiles = this.accumulator.finish(this.feedback);
      this.draft = {};
      this.draftSamples = {};
      for (const side of sides) {
      ...
      this.timing = {
        ...
        framingMs: Math.max(0, this.started - (this.requestedAt ?? this.started)),
