...
export class VirtualHand extends THREE.Group {
  ...
  applyTracked(
    ...
    this.apply(this.filteredPoints, closed, armPose, palmOrientation);
  }
...
export class Tracking {
  constructor(
    video,
    onStatus,
    { autoCalibrate = false, targetDistance = 0.55, smoothHands = autoCalibrate } = {},
  ) {
    this.video = video;
    this.onStatus = onStatus;
    this.autoCalibrate = autoCalibrate;
    this.smoothHands = smoothHands;
    ...
    try {
      ...
      this.worker.onmessage = ({ data }) => {
        ...
        if (data.type === 'result') {
          ...
          if (data.detectError)
            this.onStatus(`Hand tracking failed: ${data.detectError}`);
  ...
  scheduleFrame() {
    if (!this.active || !this.worker || this.busy || this.frameRequest !== null) return;
    const request = {};
    this.frameRequest = request;
    const worker = this.worker;
    // Prefer decoded frames, but keep a bounded fallback for a hidden preview or
    // a browser that stops issuing video callbacks. Never infer the same frame twice.
    const post = async () => {
      if (this.frameRequest !== request) return;
      this.cancelFrame();
        ...
        worker.postMessage(
          {
            ...
            trackBody: this.currentArmProfiles.size > 0,
            trackArmView: !!this.trackArmView,
    ...
    if (this.video.requestVideoFrameCallback)
      this.frameCallback = this.video.requestVideoFrameCallback(post);
    this.frameTimer = setTimeout(post, this.video.requestVideoFrameCallback ? 50 : 16);
  }
  ...
  cancelFrame() {
    ...
    this.frameCallback = this.frameTimer = this.frameRequest = null;
  }
  ...
  tick(now, hands, results = this.results) {
    ...
    const body = results.pose,
      assignments = matchHandsToBody(
        results.landmarks,
        body && results.timestamp - body.timestamp <= 180 ? body.landmarks?.[0] : null,
        this.video.videoWidth / this.video.videoHeight || 1,
      );
    ...
    results.landmarks.forEach((lm, i) => {
      ...
      if (profile) {
        ...
        h.tracked = true;
        h.calibrated = true;
        h.confidence = results.handedness?.[i]?.[0]?.score ?? 1;
        h.updated = previouslyTracked.get(h.side) === true;
        h.applyTracked(
          pose.joints.slice(3),
          fistScore(lm),
          sampleDt,
          !h.updated || !this.smoothHands,
          pose,
          palmFrame(pose.joints.slice(3), side),
        );
        h.sampleDt = sampleDt;
