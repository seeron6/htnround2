...
// GLTFLoader may expose interleaved attributes. The editable hair solver uses
// plain packed arrays, so deinterleave the imported head before binding its
// photo roots; otherwise every strand reads the wrong byte stride.
function packedGeometry(source) {
  ...
  if (source.index) geometry.setIndex(source.index.array.slice());
  return geometry;
}
...
function install(p, root, details = [], fitObject = root) {
  root.updateMatrixWorld(true);
  // Hair guides can contain long observed offsets and should not decide the
  // display scale. Fit the comparison to the deformable head mesh when one is
  // supplied, keeping the two panels at the same anatomical height.
...
function setMode(value) {
  ...
  for (const p of panels) {
    ...
    p.details.forEach((obj) => {
      // Keep the independent hair shell and fibers in neutral geometry views
      // so the silhouette comparison includes the reconstructed volume.
      // Glasses remain reserved for Texture mode because their optical shells
      // would otherwise dominate the clay/wire read.
      const isShell = obj.userData.structure === 'surface-shell';
      // The shell is a shape-view aid. In Texture mode the photographed scalp
      // atlas plus traced fibers should carry the look; showing the neutral
      // shell there creates a hard artificial hairline over the photo.
      obj.visible =
        obj.userData.structure === 'surface-shell'
          ? false
          : obj.userData.structure === 'hair-clumps'
            ? mode !== 'texture'
            : obj.userData.accessory === 'hair' || mode === 'texture';
      // Details are added after some GLB imports have already been walked by
      // the main mesh pass. Keep hair neutral in Shape/Wireframe even when a
      // loader did not expose its child meshes in that first traversal; the
      // review should show volume and strand placement without black texture
      // seams being mistaken for holes in the scalp.
      const detailMode =
        obj.userData.accessory === 'hair' && mode === 'shape' ? 'texture' : mode;
      if (obj.isMesh && obj.userData.comparisonMaterials)
        obj.material = obj.userData.comparisonMaterials[detailMode];
      obj.traverse((child) => {
        if (!child.isMesh || child.userData.accessory === 'eyeglasses') return;
        if (!child.userData.comparisonMaterials) {
          ...
          child.userData.comparisonMaterials = {
            texture,
            clay: isShell
              ? new THREE.MeshBasicMaterial({ color: 0xb7c5b6, side: THREE.DoubleSide })
        ...
        child.material = child.userData.comparisonMaterials[detailMode];
      });
    });
    p.draw();
  }
...
async function native(p) {
  if (nativeGlb && /^[a-z0-9-]+$/.test(nativeGlb)) {
    ...
    const details = [];
    let head, glassesSpec, hairSpec;
    gltf.scene.traverse((obj) => {
      if (obj.isMesh && !obj.userData.accessory && obj.userData.accessories) {
        head = obj;
        glassesSpec = obj.userData.accessories.glasses;
        hairSpec = obj.userData.accessories.hair;
      }
    ...
    if (head && hairSpec?.cap?.enabled) {
      const cap = new HeadHairSurface(head.geometry, hairSpec);
      gltf.scene.add(cap);
      details.push(cap);
      if (hairSpec.rootTriangles?.length) {
        const hairSource = packedGeometry(head.geometry),
          hair = new HeadHair(hairSource, hairSpec);
        hair.userData.comparisonSource = hairSource;
        gltf.scene.add(hair);
        details.push(hair);
      }
    }
    install(p, gltf.scene, details, head || gltf.scene);
