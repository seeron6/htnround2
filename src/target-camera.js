import {FistPoseEstimator,PinholeCamera,handApparentSpan,closureOf} from './fist-pose.js';
...
export function lateralOffset(point,depth,camera){
  return {x:(point.x*2-1)*depth*camera.kx,y:(1-point.y*2)*depth*camera.ky};
}

export class TargetTracking{
  constructor(video,onStatus,{onImpact,fovDegrees=60,targetWidth=.22,headWidth=.30,
    graceMs=160,motionFloor=.010,headRadii=[.15,.14,.09],mirrored=false,...tuning}={}){
    Object.assign(this,{video,onStatus,onImpact,fovDegrees,targetWidth,headWidth,motionFloor,headRadii,mirrored});
    this.camera=new PinholeCamera({fovDegrees,viewAspect:4/3,sourceAspect:4/3});
    this.extractor=new PunchExtractor({...DEFAULTS,graceMs,...tuning});
    this.active=false;this.inFlight=0;this.frameCallback=null;this.results=null;this.appliedTimestamp=0;
    this.rate=0;this.lastResultAt=null;
    ...
    this.lastDepth=null;this.debugSpan=0;this.debugMotion=0;this.lastHands=0;
    ...
    this.probe={results:0,rawHands:0,withWorld:0,fitOk:0,fitFail:0,lastFit:'',detectError:'',
      crudeDepth:null,crudeClosing:0,crudeTime:0};
  }
  // The extractor's knobs, surfaced as plain properties so the debug sliders can poke them.
  get minPeak(){return this.extractor.options.minPeak;}
  set minPeak(value){if(Number.isFinite(value))this.extractor.configure({minPeak:value});}
  get startClosing(){return this.extractor.options.startClosing;}
  ...
  setFov(value){if(Number.isFinite(value)&&value>20&&value<140){this.fovDegrees=value;this.camera.set({fovDegrees:value});}}
  setHeadWidth(value){if(Number.isFinite(value)&&value>.05)this.headWidth=value;}
  ...
  #label(raw){return this.mirrored?(raw==='Left'?'Right':raw==='Right'?'Left':raw):raw;}
  setHeadRadii(radii){if(Array.isArray(radii)&&radii.length===3&&radii.every(v=>Number.isFinite(v)&&v>.01))this.headRadii=[...radii];}
  get gain(){return this.headWidth/Math.max(this.targetWidth,1e-3);}
  get slots(){return this.extractor.slots;}
  ...
  get tracks(){return this.extractor.slots;}
  get stats(){return {...this.extractor.stats,frames:this.counts.frames,detected:this.counts.detected};}
  get debug(){
    const d=this.extractor.debug;
    return {hands:this.lastHands,span:this.debugSpan,motion:this.debugMotion,
      depth:this.lastDepth,range:d.r,closing:d.closing??0,phase:d.phase??'idle',
      peak:d.peak??0,travel:d.travel??0,minDepth:d.minRange??null,key:d.slot??'—'};
  }
  ...
  async start(deviceId=''){
    ...
    try{
      ...
      const video={width:{ideal:640},height:{ideal:480},frameRate:{ideal:60,min:30}};
      if(deviceId)video.deviceId={exact:deviceId};else video.facingMode={ideal:'user'};
      this.stream=await navigator.mediaDevices.getUserMedia({video,audio:false});
      this.video.srcObject=this.stream;await this.video.play();
      this.worker=new Worker('/target-worker.js');
      await new Promise((resolve,reject)=>{
        const timeout=setTimeout(()=>reject(new Error('Target CV models timed out while loading.')),25000);
      ...
      this.worker.onmessage=({data})=>{
        ...
        if(data.type==='result'){
          if(!this.results||data.timestamp>this.results.timestamp)this.results=data;
          const gap=data.timestamp-(this.lastResultAt??data.timestamp);
          if(gap>0)this.rate=this.rate?this.rate+((1000/gap)-this.rate)*.1:1000/gap;
      ...
      this.active=true;this.onStatus('Target camera live.');this.scheduleFrame();
  ...
  stop(){
    ...
    this.worker?.terminate();this.worker=null;this.inFlight=0;this.results=null;this.appliedTimestamp=0;
    this.frameCallback=null;this.extractor.reset();this.estimators.clear();this.rate=0;this.lastResultAt=null;
    this.lastDepth=null;this.debugSpan=0;this.lastHands=0;
  ...
  scheduleFrame(){
    ...
    const post=async()=>{
      ...
      if(this.video.readyState<2){this.scheduleFrame();return;}
      this.inFlight++;
      try{
  ...
  estimatorFor(key){
    ...
    if(!estimator){estimator=new FistPoseEstimator({camera:this.camera});this.estimators.set(key,estimator);}
    return estimator;
  }
  #solveOptions(){return {gain:this.gain,headRadii:this.headRadii};}
  tick(now){
    ...
    this.counts.frames++;
    const sourceAspect=this.video.videoWidth&&this.video.videoHeight?this.video.videoWidth/this.video.videoHeight:4/3;
    // No cover crop: this camera is a measuring instrument, read in its own frame, not a backdrop.
    this.camera.set({fovDegrees:this.fovDegrees,viewAspect:sourceAspect,sourceAspect});
    this.debugMotion=results.motion?.energy??0;
    const raw=results.landmarks??[];
    this.lastHands=raw.length;
    this.probe.results++;this.probe.rawHands=raw.length;
    ...
    const detections=raw.map((landmarks,index)=>({
      ...
      centre:landmarks?.length===21
        ?{x:mu((landmarks[5].x+landmarks[17].x)/2),y:(landmarks[5].y+landmarks[17].y)/2}
        :{x:.5,y:.5},
    })).filter(detection=>detection.span);
    this.probe.withWorld=detections.filter(detection=>detection.world?.length===21).length;
    ...
    const nearest=detections.reduce((best,d)=>!best||d.knuckles>best.knuckles?d:best,null);
    if(nearest&&nearest.knuckles>1e-4){
      const span=this.estimators.get(this.#label(nearest.raw))?.shape?.span||KNUCKLE_SPAN_PRIOR;
      const crude=span/(2*nearest.knuckles*this.camera.kx);
      const elapsed=(results.timestamp-this.probe.crudeTime)/1000;
      this.probe.crudeClosing=this.probe.crudeDepth!==null&&elapsed>1e-3&&elapsed<.5
        ?(this.probe.crudeDepth-crude)/elapsed:0;
      this.probe.crudeDepth=crude;this.probe.crudeTime=results.timestamp;
    }else{this.probe.crudeDepth=null;this.probe.crudeClosing=0;}
    ...
    else{this.lastDepth=null;this.debugSpan=0;}

    const batch=usable.map(d=>({u:d.centre.x,v:d.centre.y,span:d.span,det:d}));
    const assignment=this.extractor.assign(batch,results.timestamp);
    for(const item of batch){
      const slot=assignment.get(item),d=item.det;
      ...
      if(solved){
        this.probe.fitOk++;this.probe.lastFit='';
        this.lastDepth=-solved.centre[2];this.debugSpan=d.span;
