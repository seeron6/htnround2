...
export class LiveArms {
  ...
  get state() {
    return {
      ...
      frames: this.frames,
      detectedHands: this.contactResults?.landmarks?.length ?? 0,
      trackingError: this.tracking.results?.detectError || null,
      poseError: this.poseError || null,
      frameAgeMs: this.maskTime == null ? null : performance.now() - this.maskTime,
  ...
  resetWorker() {
    ...
    this.poseReady = this.poseBusy = false;
    this.pose = null;
    this.poseError = null;
    this.lastPoseSent = -Infinity;
    this.key = null;
    this.failedKey = null;
    ...
    try {
      ...
      this.worker.onmessage = ({ data }) => {
        ...
        if (data.type === 'ready') {
          ...
          this.maskReceivedAt = performance.now();
      ...
      if (this.source === 'punch') this.startPoseWorker(epoch);
    } catch (error) {
      this.fail(error.message, key);
    }
  }

  startPoseWorker(epoch) {
    // Elbows improve the sleeve cutout, but a slow pose model must never block
    // hand detection or segmentation. Keep one optional pose frame in flight.
    try {
      const worker = new Worker('/arm-pose-worker.js');
      this.poseWorker = worker;
      worker.onmessage = ({ data }) => {
        if (epoch !== this.epoch) return;
        if (data.type === 'ready') this.poseReady = true;
        else if (data.type === 'pose') {
          this.pose = data;
          this.poseBusy = false;
        } else if (data.type === 'error') {
          this.poseError = data.message;
      ...
      worker.onerror = (event) => {
        if (epoch !== this.epoch) return;
        this.poseError = event.message;
        this.poseReady = false;
        worker.terminate();
      };
      worker.postMessage({ type: 'init', origin: location.origin });
    } catch (error) {
      this.poseError = error.message;
    }
  }
  ...
  update(time, hidden = false) {
    ...
    const sourceFrame = isBody ? video : results?.frame;
    const sourceTime = isBody ? time : results?.timestamp;
    const freshFrame = isBody
      ? video.currentTime !== this.lastVideoTime
      : sourceFrame &&
        sourceTime !== this.lastTrackingTimestamp &&
        time - (this.tracking.receivedAt ?? sourceTime) < 250;
    if (
      !isBody &&
      freshFrame &&
      this.poseReady &&
      !this.poseBusy &&
      sourceTime - this.lastPoseSent >= 150
    ) {
      this.poseBusy = true;
      this.lastPoseSent = sourceTime;
      const epoch = this.epoch;
      createImageBitmap(sourceFrame)
        .then((bitmap) => {
          if (epoch !== this.epoch) {
            bitmap.close();
            return;
          ...
          this.poseWorker.postMessage(
            { type: 'frame', bitmap, timestamp: sourceTime },
            [bitmap],
          );
        })
        .catch((error) => {
    ...
    if (
      ...
      this.lastTrackingTimestamp = sourceTime;
      const epoch = this.epoch;
      // The front-view landmarks and pixels must be from one exposure. Sampling
      // the newer live video here used to erase moving hands from their own mask.
      createImageBitmap(sourceFrame)
        .then((bitmap) => {
          if (epoch !== this.epoch) {
            bitmap.close();
          ...
          this.worker.postMessage(
            {
              ...
              landmarks: isBody ? [] : results.landmarks,
              pose:
                !isBody && this.pose && sourceTime - this.pose.timestamp < 300
    ...
    if (
      !this.mask ||
      !this.frame ||
      time - this.maskReceivedAt > 400 ||
      video.readyState < 2
    ) {
      this.canvas.hidden = true;
      if (this.ready)
        this.status(
          !isBody && results?.detectError
            ? `Hand tracking failed: ${results.detectError}. Reconnect the webcam to retry.`
            : !isBody &&
                (!results ||
                  time - (this.tracking.receivedAt ?? results.timestamp) > 1000)
              ? 'Waiting for fresh camera frames. Check the preview; reconnect if it is frozen.'
              : 'Waiting for the live arm cutout…',
        );
      return;
    }
    ...
    let message;
    if (!isBody && results?.detectError)
      message = `Hand tracking failed: ${results.detectError}. Reconnect the webcam to retry.`;
    else if (this.visiblePixels && time - this.maskTime > 400)
      message = 'Live arms are delayed. Try Skeleton arms while the camera catches up.';
    else if (this.visiblePixels)
      message = isBody
      ...
      message = this.contactResults?.landmarks?.length
        ? 'Hands detected, but the cutout is empty. Keep hands clear of your face and shirt in even light.'
        : 'No hands detected yet. Show an open hand fully inside the camera preview, then close your fist.';
    this.status(message);
  }
