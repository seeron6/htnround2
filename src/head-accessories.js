    ...
    spec.temples.forEach((points, i) => {
      ...
      add(
        sweep(
          ...
          (t) => ({
            axis: new THREE.Vector3(0, 1, 0),
            // The source profile shows a broad acetate temple that stays
            // visually substantial behind the hinge. A heavy taper turns it
            // into a floating ink line at profile angles, so only the final
            // ear-hook transition narrows.
            width: width * (1 - 0.52 * THREE.MathUtils.smoothstep(t, 0.68, 0.98)),
