...
export const SOURCE_W = 1280,
  SOURCE_H = 720;
export const NOISE_X = 2.5 / SOURCE_W,
  NOISE_Y = 2.5 / SOURCE_H; // ~2.5 px landmark jitter
export const REL_Z_NOISE = 0.03; // MediaPipe relative-z jitter
export const WORLD_SHAPE_NOISE = 0.004,
  WORLD_ROT_NOISE_DEG = 3;
...
export const TRUTH_FIST = (() => {
  ...
  p[0] = [0.002, -0.061, -0.007];
  const k = [
    [5, 0.046, 0.02, 0.003],
    [9, 0.015, 0.025, 0.001],
    [13, -0.015, 0.022, -0.003],
    [17, -0.044, 0.015, -0.006],
  ];
  ...
  for (const [i, x] of [
    ...
    p[i + 1] = [x * 1.04, 0.037, -0.006];
    p[i + 2] = [x * 1.01, 0.021, -0.027];
    p[i + 3] = [x * 0.94, 0.001, -0.03];
  }
  p[1] = [0.044, -0.033, -0.011];
  p[2] = [0.055, -0.009, -0.018];
  p[3] = [0.042, 0.009, -0.025];
  p[4] = [0.013, 0.014, -0.029];
...
export const TRUTH_OPEN = (() => {
  ...
  for (const [i, x] of [
    ...
    p[i + 1] = [x * 1.03, 0.056, 0.005];
    p[i + 2] = [x * 1.05, 0.082, 0.007];
    p[i + 3] = [x * 1.06, 0.104, 0.007];
  }
  return p;
})();
...
const rx = (p, t) => [
  p[0],
  p[1] * Math.cos(t) - p[2] * Math.sin(t),
  p[1] * Math.sin(t) + p[2] * Math.cos(t),
];
const ry = (p, t) => [
  p[0] * Math.cos(t) + p[2] * Math.sin(t),
  p[1],
...
export const rotate = (p, { pitch = 0, yaw = 0, roll = 0 }) =>
  ry(rx(rz(p, roll), pitch), yaw);
...
export function observe(pose, camera, random, { noise = true } = {}) {
  ...
  const truthPoints = oriented.map((p) => [
    p[0] + position[0],
    p[1] + position[1],
    p[2] + position[2],
  ]);
  ...
  const landmarks = truthPoints.map((p) => {
    const d = -p[2];
    const view = {
      x: 0.5 + p[0] / (2 * d * camera.kx),
      y: 0.5 - p[1] / (2 * d * camera.ky),
    };
    // invert the cover crop so the estimator's own cover() puts it back
    const { sourceAspect: s, viewAspect: v } = camera;
    const src =
      s > v
        ? { x: 0.5 + ((view.x - 0.5) * v) / s, y: view.y }
        : { x: view.x, y: 0.5 + ((view.y - 0.5) * s) / v };
    return {
      x: src.x + n() * NOISE_X,
      y: src.y + n() * NOISE_Y,
      z: (d - depth) / (2 * depth * camera.kx) + n() * REL_Z_NOISE,
    };
  });

  const rerr = {
    pitch: (n() * WORLD_ROT_NOISE_DEG * Math.PI) / 180,
    yaw: (n() * WORLD_ROT_NOISE_DEG * Math.PI) / 180,
  };
  ...
  const worldLandmarks = oriented.map((p) => {
    ...
    return {
      x: (q[0] + n() * WORLD_SHAPE_NOISE) * WORLD_SIGNS[0],
      y: (q[1] + n() * WORLD_SHAPE_NOISE) * WORLD_SIGNS[1],
      z: (q[2] + n() * WORLD_SHAPE_NOISE) * WORLD_SIGNS[2],
      visibility: 1,
    };
  });
  ...
  const truthCentre = [0, 1, 2].map((a) =>
    truthPoints.reduce((sum, q) => sum + q[a] / 21, 0),
  );
  return {
    landmarks,
    worldLandmarks,
    truth: { points: truthPoints, centre: truthCentre, knuckleNormal: kn, closure },
  };
}
