...
import sponsor_perception as perception
...
class FaceVoice(unittest.TestCase):
    ...
    def test_cast_changes_the_writing_even_when_audio_is_off(self):
        ...
        self.assertEqual(
            other['messages'][0]['content'], service.FACE + dialogue.direction([])
        )
    ...
    def test_a_punch_triggered_turn_hears_the_room_but_never_instead_of_the_person(
        ...
        ask, room = body['messages'][-2:]
        self.assertEqual(
            room['content'][0]['input_audio']['data'], 'data:;base64,UklGRg=='
        )
        self.assertNotIn('Nobody is asking you anything', ask['content'])
        self.assertIn('answer what they said first', ask['content'])
    ...
    def test_understood_speech_and_actions_are_returned_alongside_native_voice(self):
        self.keys()
        notes = {
            'heard': 'Call me Robin. This is my blue mug.',
            'seen': 'They hold up a blue mug.',
        }
        Upstream.script = {'mood': {'emotion': 'amused', 'intensity': 0.5, **notes}}
        got = self.turn(remember=True, frames=['QUJD'] * 4, audioWav='UklGRg==')
        self.assertEqual(next(d for n, d in got if n == 'perception'), notes)
        self.assertEqual(got[-1][0], 'done')
        self.assertIn('audio', [n for n, _ in got])
        self.assertIn('expression', [n for n, _ in got])
        self.assertEqual(len(self.spoken()), 1)
        self.assertEqual(len(self.moods()), 1)  # memory shares the expression call
        asked = self.moods()[0]
        self.assertEqual(asked['messages'][1]['content'][0]['type'], 'video')
        self.assertEqual(asked['messages'][-1]['content'][0]['type'], 'input_audio')
        self.assertIn('heard', asked['tools'][0]['function']['parameters']['required'])

    def test_room_words_are_remembered_and_coach_can_remember_without_an_expression(
        self,
    ):
        self.keys()
        Upstream.script = {
            'mood': {
                'emotion': 'smug',
                'intensity': 0.4,
                'heard': 'What did I show you?',
                'seen': '',
            }
        }
        got = self.turn(remember=True, roomWav='UklGRg==', mode='coach')
        self.assertEqual(
            next(d for n, d in got if n == 'perception')['heard'],
            'What did I show you?',
        )
        self.assertNotIn('expression', [n for n, _ in got])
        self.assertEqual(
            self.moods()[0]['messages'][-1]['content'][0]['type'], 'input_audio'
        )
        Upstream.seen = []
        got = self.turn(remember=True, audioWav='UklGRg==', expressions=False)
        self.assertIn('perception', [n for n, _ in got])
        self.assertNotIn('expression', [n for n, _ in got])

    def test_perception_failure_does_not_fail_speech_or_fabricate_memory(self):
        self.keys()
        Upstream.script = {'mood': 'refused'}
        got = self.turn(remember=True, audioWav='UklGRg==')
        self.assertNotIn('perception', [n for n, _ in got])
        self.assertIn('audio', [n for n, _ in got])
        self.assertEqual(got[-1][0], 'done')
        notes = json.dumps({'heard': 'invented speech', 'seen': 'invented observation'})
        self.assertEqual(perception.read(notes, {}), {'heard': '', 'seen': ''})
        for malformed in ('{', '[]', 'null', '42', '{}'):
            self.assertIsNone(perception.read(malformed, {}))
        bounded = perception.read(
            json.dumps({'heard': 'h' * 500, 'seen': 's' * 500}),
            {'audioWav': 'abc', 'frames': ['abc']},
        )
        self.assertEqual((len(bounded['heard']), len(bounded['seen'])), (200, 160))
