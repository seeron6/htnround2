...
const source = readFileSync(
  fileURLToPath(new URL('../public/target-motion.js', import.meta.url)),
  'utf8',
);
const worker = {};
new Function('self', 'OffscreenCanvas', source)(worker, class {});
const { componentsOf, blobFlow } = worker.TargetMotion;
...
test('frame differencing finds the moving mass as one blob near its true position', () => {
  const a = frame({ cx: 30, cy: 36, half: 10 }),
    b = frame({ cx: 34, cy: 36, half: 10 });
  const { blobs } = componentsOf(diffOf(a, b), W, H);
  assert.ok(blobs.length >= 1, 'the moving square is a component');
  const blob = blobs[0],
    cx = blob.sx / blob.mass;
  assert.ok(
    Math.abs(cx - 32) < 6,
    `centroid between the two positions (got x=${cx.toFixed(1)})`,
  );
});
...
test('similarity flow reads pure translation as translation, not expansion', () => {
  ...
  assert.ok(
    Math.abs(flow.scalePerFrame) < 0.12,
    `no phantom looming (got ${flow.scalePerFrame.toFixed(3)})`,
  );
});

test('similarity flow reads expansion as looming with the right sign', () => {
  const a = frame({ cx: 48, cy: 36, half: 10 }),
    b = frame({ cx: 48, cy: 36, half: 13 });
  const { labels, blobs } = componentsOf(diffOf(a, b), W, H);
  const flow = blobFlow(a, b, W, H, labels, blobs[0]);
  assert.ok(flow, 'enough texture to match');
  assert.ok(
    flow.scalePerFrame > 0.12,
    `a growing square is approaching (got ${flow.scalePerFrame.toFixed(3)})`,
  );
  assert.ok(
    Math.abs(flow.du) < 2.5 && Math.abs(flow.dv) < 2.5,
    'and it is not misread as travel',
  );
  const shrink = blobFlow(
    b,
    a,
    W,
    H,
    componentsOf(diffOf(b, a), W, H).labels,
    componentsOf(diffOf(b, a), W, H).blobs[0],
  );
  assert.ok(
    shrink && shrink.scalePerFrame < -0.12,
    `a shrinking one is retreating (got ${shrink?.scalePerFrame.toFixed(3)})`,
  );
});
