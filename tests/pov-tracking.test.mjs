...
test('first-person projection moves a shrinking fist away from the body camera without mirroring it', () => {
  const landmarks = hand(0.72, 0.32, 0.08),
    guard = hand(0.72, 0.32, 0.16),
    projected = projectPovHand(landmarks, {
      guardScale: handScale(guard),
      guardDepth: 0.25,
      targetDistance: 0.65,
      fovDegrees: 60,
      viewAspect: 16 / 9,
  ...
  assert.ok(Math.abs(projected.depth - 0.5) < 1e-9);
  assert.ok(
    projected.points[9].x > 0,
    'right side of the phone image stays right in the scene',
  );
  assert.ok(
    projected.points[9].z < -0.45,
    'smaller apparent hand is farther from the body camera',
  );
});

test('cover projection matches the full-frame cutout crop', () => {
  assert.deepEqual(coverPoint({ x: 0.25, y: 0.4, z: 0 }, 2, 1), { x: 0, y: 0.4, z: 0 });
  assert.deepEqual(coverPoint({ x: 0.4, y: 0.25, z: 0 }, 1, 2), { x: 0.4, y: 0, z: 0 });
});

test('two hand tracks keep identity when their image positions cross', () => {
  const left = { side: -1, tracked: true, imageCenter: { x: 0.55, y: 0.4 } },
    right = { side: 1, tracked: true, imageCenter: { x: 0.45, y: 0.4 } },
    detections = [{ center: { x: 0.56, y: 0.4 } }, { center: { x: 0.44, y: 0.4 } }],
    match = matchPovHands(detections, [left, right]);
  assert.equal(match.get(0), left);
  assert.equal(match.get(1), right);
});
...
test('corner arcs calibrate when the cutout fills them, gaps allowed', () => {
  const tracking = new PovTracking({ videoWidth: 1280, videoHeight: 720 }, () => {}, {
    estimator: 'legacy',
    holdMs: 300,
  });
  const hands = [{ side: -1 }, { side: 1 }],
    targets = tracking.guardTargets;
  const guard = (side) => fist(targets[side].x, targets[side].y - 0.07, 0.35);
  let t = 1000;
  // Fists posed correctly but the mask barely reaches the arcs: no progress.
  for (let i = 0; i < 10; i++)
    tracking.applyResults(
      {
        landmarks: [guard('-1'), guard('1')],
        coverage: { '-1': 0.2, 1: 0.2 },
        handedness: [],
        timestamp: (t += 50),
      },
      hands,
  ...
  assert.ok(
    tracking.guard.get(-1).scale > 0 && tracking.guard.get(1).scale > 0,
    'guard scale captured per hand',
  );
  assert.ok(hands[0].calibrated && hands[1].calibrated);
  assert.deepEqual(
    hands[1].guardCenter,
    targets['1'],
    'the arc becomes the reacquisition anchor',
  );
});
...
test('a fist still below the frame edge cannot set the guard even with arc coverage', () => {
  ...
  for (let i = 0; i < 15; i++)
    tracking.applyResults(
      {
        landmarks: [
          fist(targets['-1'].x, 1.02, 0.35),
          fist(targets['1'].x, targets['1'].y - 0.07, 0.35),
        ],
        coverage: { '-1': 0.7, 1: 0.7 },
        handedness: [],
        timestamp: (t += 50),
      },
      hands,
...
test('image samples for render-rate prediction carry velocity and reset on reacquisition', () => {
  ...
  const hands = [{ side: -1 }, { side: 1 }],
    targets = tracking.guardTargets;
  let t = 1000;
  const pose = (side) => fist(targets[side].x, targets[side].y - 0.07, 0.35);
  tracking.applyResults(
    { landmarks: [pose('-1'), pose('1')], handedness: [], timestamp: t },
    hands,
    16 / 9,
    t,
  );
  ...
  tracking.applyResults(
    { landmarks: [pose('-1'), pose('1')], handedness: [], timestamp: (t += 50) },
    hands,
    16 / 9,
    t,
  );
  assert.ok(
    hands[0].imagePrev && hands[0].imageCur.time > hands[0].imagePrev.time,
    'consecutive samples pair up',
  );
...
test('skipCalibration fabricates a completed ritual from live hands', () => {
  ...
  assert.ok(
    tracking.guard.get(-1).scale > 0 && tracking.guard.get(1).scale > 0,
    'guard scales synthesized',
  );
  assert.ok(hands[0].calibrated && hands[1].calibrated);
  assert.deepEqual(
    hands[1].guardCenter,
    targets['1'],
    'anchors set as a real completion would',
  );
  // Works with no hands ever seen, too: falls back to a default scale.
  const cold = new PovTracking({ videoWidth: 1280, videoHeight: 720 }, () => {}, {
    estimator: 'legacy',
  });
