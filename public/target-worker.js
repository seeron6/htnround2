...
let hands,motionCanvas,motionContext,previousGrey=null,previousStamp=0,lastStamp=0;
...
function greyOf(bitmap,height){
  ...
  for(let i=0;i<grey.length;i++)grey[i]=(frame[i*4]*77+frame[i*4+1]*150+frame[i*4+2]*29)>>8;
  return grey;
}
...
function componentsOf(diff,width,height){
  const labels=new Int32Array(diff.length),stack=[];
  const blobs=[];
  for(let start=0;start<diff.length;start++){
    if(!diff[start]||labels[start])continue;
    const id=blobs.length+1;
    let mass=0,cells=0,sx=0,sy=0,sxx=0,syy=0,minX=width,maxX=0,minY=height,maxY=0;
    labels[start]=id;stack.push(start);
    while(stack.length){
      const index=stack.pop(),weight=diff[index];
      const x=index%width,y=(index-x)/width;
      mass+=weight;cells++;sx+=x*weight;sy+=y*weight;sxx+=x*x*weight;syy+=y*y*weight;
      if(x<minX)minX=x;if(x>maxX)maxX=x;if(y<minY)minY=y;if(y>maxY)maxY=y;
      if(x>0&&diff[index-1]&&!labels[index-1]){labels[index-1]=id;stack.push(index-1);}
      if(x<width-1&&diff[index+1]&&!labels[index+1]){labels[index+1]=id;stack.push(index+1);}
      if(y>0&&diff[index-width]&&!labels[index-width]){labels[index-width]=id;stack.push(index-width);}
      if(y<height-1&&diff[index+width]&&!labels[index+width]){labels[index+width]=id;stack.push(index+width);}
    }
    if(cells>=MIN_BLOB_CELLS)blobs.push({id,mass,cells,sx,sy,sxx,syy,minX,maxX,minY,maxY});
  }
...
function matchPoint(prev,cur,width,height,x,y){
  const sad=(dx,dy)=>{
    let total=0;
    for(let py=-FLOW_PATCH;py<=FLOW_PATCH;py++)for(let px=-FLOW_PATCH;px<=FLOW_PATCH;px++){
      const ax=x+px,ay=y+py,bx=x+px+dx,by=y+py+dy;
      if(ax<0||ax>=width||ay<0||ay>=height||bx<0||bx>=width||by<0||by>=height)return Infinity;
      total+=Math.abs(prev[ay*width+ax]-cur[by*width+bx]);
    }
    return total;
  };
  let bestX=0,bestY=0,bestCost=sad(0,0);
  for(let step=Math.ceil(FLOW_SEARCH/2);step>=1;step=step>1?Math.ceil(step/2):0){
    let improved=true;
    while(improved){
      improved=false;
      for(const [dx,dy] of [[step,0],[-step,0],[0,step],[0,-step]]){
        const cost=sad(bestX+dx,bestY+dy);
        if(cost<bestCost){bestCost=cost;bestX+=dx;bestY+=dy;improved=true;}
      }
    }
    if(step===1)break;
  }
...
function blobFlow(prev,cur,width,height,labels,blob){
  const cx=blob.sx/blob.mass,cy=blob.sy/blob.mass;
  let n=0,sumDx=0,sumDy=0,sumRD=0,sumRR=0,sumX=0,sumY=0;
  const points=[];
  for(let y=blob.minY;y<=blob.maxY;y+=FLOW_STEP)for(let x=blob.minX;x<=blob.maxX;x+=FLOW_STEP){
  ...
  for(const [x,y] of points){
    ...
    n++;sumDx+=dx;sumDy+=dy;sumX+=rx;sumY+=ry;
    sumRD+=rx*dx+ry*dy;sumRR+=rx*rx+ry*ry;
  }
  if(n<4||sumRR<1e-6)return null;
  // Least squares with the radial coordinates re-centred on the sampled points.
  const meanDx=sumDx/n,meanDy=sumDy/n,meanX=sumX/n,meanY=sumY/n;
  const s=(sumRD-n*meanX*meanDx-n*meanY*meanDy+0)/Math.max(sumRR-n*(meanX*meanX+meanY*meanY),1e-6);
  return {du:meanDx-s*meanX,dv:meanDy-s*meanY,scalePerFrame:s,samples:n};
}
...
function motionOf(bitmap,timestamp){
  ...
  if(!previousGrey||previousGrey.length!==grey.length){
    previousGrey=grey;previousStamp=timestamp;
    return null;
  }
  const diff=new Uint8Array(grey.length);
  let total=0,sx=0,sy=0,peak=0;
  for(let i=0;i<grey.length;i++){
    ...
    total+=difference;
    const x=i%MOTION_W;
    sx+=x*difference;sy+=((i-x)/MOTION_W)*difference;
    if(difference>peak)peak=difference;
  }
  const dtSeconds=Math.max((timestamp-previousStamp)/1000,1e-3);
  const {labels,blobs}=total>0?componentsOf(diff,MOTION_W,height):{labels:null,blobs:[]};
  const out=[];
  for(const blob of blobs){
    const cx=blob.sx/blob.mass,cy=blob.sy/blob.mass;
    const varX=Math.max(blob.sxx/blob.mass-cx*cx,0),varY=Math.max(blob.syy/blob.mass-cy*cy,0);
    const flow=blobFlow(previousGrey,grey,MOTION_W,height,labels,blob);
    out.push({
      u:(cx+.5)/MOTION_W,v:(cy+.5)/height,
  ...
  previousGrey=grey;previousStamp=timestamp;
  if(total<=0)return {energy:0,x:.5,y:.5,peak:0,blobs:out};
  return {energy:total/(grey.length*255),x:(sx/total+.5)/MOTION_W,y:(sy/total+.5)/height,peak:peak/255,blobs:out};
}
...
self.onmessage=async({data})=>{
  try{
    if(data.type==='init'){
      ...
      hands=await create(HandLandmarker,files,{baseOptions:{modelAssetPath:`${data.origin}/models/hand_landmarker.task`},
        runningMode:'VIDEO',numHands:2,minHandDetectionConfidence:.35,minTrackingConfidence:.3});
    ...
    try{
      ...
      let result=null,detectError='';
      try{result=hands.detectForVideo(data.bitmap,lastStamp);}
      catch(error){detectError=error?.message??String(error);}
      const motion=motionOf(data.bitmap,data.timestamp);
      self.postMessage({type:'result',
        landmarks:result?.landmarks??[],worldLandmarks:result?.worldLandmarks??[],
        handedness:result?.handedness??result?.handednesses??[],
        motion,detectError,timestamp:data.timestamp});
