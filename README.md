...
Choose **Skeleton arms**, **Live CV arms (Jace)**, or **My 3D arms** during calibration or under **Camera → Arm appearance**. The optional **Scan / customize my arms** button fits bare/sleeved first-person presets from an eight-second local capture, with editable colors, photographed markings, rings and watches. See [Arm modes and quick scan](docs/ARM_MODES.md) for the measured timing, controls and limits.

For real hands and sleeves in the punching view, use **Camera → Live CV arms (Jace)**.
The in-app **How to set up a separate camera** guide explains the two-camera setup;
see [Real arms and body-camera setup](docs/ARM_CAMERA_SETUP.md) for the full instructions.
...
1. Choose **Record / upload head video**. Use **Record 360°** with the laptop webcam, or **Upload video** for MP4, WebM, or another format this browser can decode. Video upload automatically continues through extraction and reconstruction, using the filename as the saved head name (rename it later). Video import extracts up to 220 samples locally. The original recording and masked head PNG frames are saved in the private local scan folder. The scan panel measures elapsed time from selecting the video until the finished model is detected, including upload and status checks; scene loading is listed separately. The original recording is not sent to the AI service.
2. Glasses can be reconstructed as a separate estimated accessory; removing them gives clearer eye and skin evidence. Keep a neutral expression, and keep hair, ears and chin inside the frame. Start facing the camera with your eyes open, looking toward the lens for a brief pause. Slowly turn the head and torso together through a full rotation, ending at the front. Alternatively, have someone film around a seated, still person with a phone. Avoid changing lenses/zoom or tilting the head. Live capture stops at 120 seconds or 240 accepted frames. **Finish recording** turns the camera off and starts reconstruction automatically when enough views are saved.
...
4. Reconstruction starts automatically after a live recording, video import, or photo import finishes with enough usable views, using the selected engine and AI refinement setting. Stopping an import cancels automatic reconstruction. **Manual build / retry** remains available for saved scans or failed builds. The local pipeline recovers cameras with COLMAP, triangulates reliable facial landmarks, and fits a complete MakeHuman head template using a regularized global warp. Catmull-Clark subdivision preserves smooth facial loops, ears, skull and neck. A smooth hair envelope fits captured silhouettes while independent AI analysis runs. Reserved views check raw landmark reprojection, not independent ground truth or the accuracy of every template vertex.
...
Webcam punches use the trajectory pipeline ported from [`jace/cv` at `d0c1763`](https://github.com/akashngb/punching-face/commit/d0c1763): closed-fist straights, left/right hooks, uppercuts and overhands map onto the loaded mesh along their measured travel direction. The same camera worker supplies motion evidence through brief landmark dropouts. Each physical approach produces one event, which drives the existing live/clay tissue response. Enable **Mirrored camera feed** only for a camera that delivers flipped pixels. See [the CV integration notes](docs/target-cv-pipeline.md) for the mapping, diagnostics and validation limits.
...
`npm run dev` starts Vite (5173), the reconstruction API (5174), the physics service (5175), the OMNI realtime relay (5177) and the sponsor services (5176) that back the Face/Arena panel, in one process supervisor. If 5176 is already taken (someone ran `npm run sponsors`), that one is left running; if the sponsor relay exits, the rest keeps going.

**OMNI Live judges start here:** [TRACKS/OMNI_JUDGES.md](TRACKS/OMNI_JUDGES.md) (scenario, rubric map, measured numbers), [DEMO.md](DEMO.md) (the 3-minute script), and `.venv/bin/python scripts/omni_preflight.py` (re-checks seeing, hearing, tone, expression and safety against the live model in about 20 s).
