...
The panel now sends `remember: true`. `sponsor_perception.py` extends the parallel expression call with two short
observations: the intelligible words OMNI heard and what it saw in the current clip. The relay returns a private
`perception` event; `src/sponsors/dialogue-memory.js` pairs those observations with the actual reply for the next
six exchanges. Spoken turns therefore keep understood words rather than a `[spoken question]` placeholder.
The speech stream still starts independently of that call. Coach mode also gets these notes. If perception fails
or misses the turn deadline, the reply continues, but that turn has no recovered audio/visual memory. Observations
can be imperfect; new evidence and spoken corrections override them. Exact words are never inferred from lips.

Questions and things shown to the camera take priority over automatic punch banter. A punch-triggered turn can
contain a spoken question: room audio arrives last, as on a direct spoken turn. The microphone resumes 450 ms
after playback, while unsolicited punch replies retain their seven-second spacing. Echo rejection still applies
during playback in Ambient mode; Interrupt mode allows talking over it. Speech beginning to open the microphone
gate also suppresses an automatic punch reply or cached grunt.

Memory stays in the panel's session, with no raw recordings or frames in conversation history and no observation
text in Sentry. Stopping the face or changing cast/mode clears it. Turning vision off removes visual notes;
turning room hearing off removes room-speech notes. Direct microphone speech remains a separate input.
Run `.venv/bin/python scripts/omni_dialogue_check.py` for a live synthetic name/object recall, correction and
room-question check; evidence is written to `.local/omni-dialogue-evidence/`.
