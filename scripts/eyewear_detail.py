...
def refine_glasses_detail(folder, spec, advice, rec, center, basis, scale):
    ...
    for old in spec['rims']:
        ...
        # Keep the fitted acetate section slightly inside the antialiased dark
        # band. The earlier enlargement made the independent rim read as a
        # heavy black outline and obscured the photographed eyes.
        widths.append(np.clip(w * 0.88, 0.0016, 0.0058).tolist())
    ...
    result = {
        **spec,
        ...
        'rims': rims,
        'rimWidths': widths,
        # Profile references show a slim acetate edge rather than a deep
        # block. Keep enough depth for a highlight while preserving the
        # photographed front contour.
        'rimDepth': 0.0038,
        'bridgeWidth': 0.0048,
        'bridgeDepth': 0.0042,
        'lensThickness': 0.0014,
        'detailEvidence': {
            'source': im.name,
